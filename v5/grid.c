...
void place_snake (Grid * grille, Snake *s){
    ...
    for (i = 0; i< SNAKE_SIZE; i++){
        grille->grid[s->pos[i].y][s->pos[i].x] = SNAKE; /* Récupère les positions des cases du serpent et modifie la grille en ajoutant 's'*/
    }
}
...
Element move_snake (Grid * grille, Snake * s){
    ...
    tete = grille->grid[s->pos[0].y][s->pos[0].x];
