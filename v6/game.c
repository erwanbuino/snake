...
int main(int argc, char * const *argv) {
    ...
    Snake * serpent = new_snake(BOTTOM);
    ...
    add_segment(serpent,1,4);
    add_segment(serpent,1,3);
    add_segment(serpent,1,2);
    add_segment(serpent,1,1);
        ...
        do {
            ...
            switch (next_option) { /* Analyse les options données en ligne de commande */
                ...
                case 'i': /* L'utilisateur entre un fichier de carte en paramètre */
                    ...
                    grille = initialise(fichier, serpent); /* Initialise la grille avec le fichier fourni */
    ...
    if (optionI  == 0){  /*Vérifie s'il n'y a pas eu d'option -i*/
        ...
        grille = initialise(fichier, serpent); /* Initialise la grille avec celle par défaut */
    }
    ...
    )
    {
        ...
        if(loop_count == 0){
            tete = move_snake(grille, serpent); /* Déplace le serpent dans la direction indiquée dans de la champ dir de sa structure*/
            switch (tete)
            {
